...
const EventSummarySchema = z.object({
  ...
  example: z
    .string()
    .describe(
      'A concise Java code snippet (under 15 lines) showing basic event handling with @SubscribeEvent annotation, demonstrating 1-2 key fields/methods from the event object. Use standard MinecraftForge patterns.',
    ),
});
...
const catOf = (p: string): string =>
  (p.split('/net/minecraftforge/')[1] ?? p).split('/')[0].replace(/v\d+$/, '') || 'misc';
